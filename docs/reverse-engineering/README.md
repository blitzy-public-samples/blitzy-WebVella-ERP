...
| **Plugins (7)** | `WebVella.Erp.Plugins.{Approval, Crm, Mail, MicrosoftCDM, Next, Project, SDK}` | Feature modules: each ships a `*Plugin._.cs` bootstrap, dated migration partials, and `Controllers`/`Services`/`Components`/`Hooks`/`Jobs` as applicable |
...
| **plugin** | A feature module implemented as an **`ErpPlugin` subclass** (`WebVella.Erp/ErpPlugin.cs`). The suite documents **seven** plugins: Approval, Crm, Mail, MicrosoftCDM, Next, Project, and SDK. |
| **plugin bootstrap** | The plugin's primary partial-class file, conventionally named **`*Plugin._.cs`**, whose initialization logic (`ProcessPatches`) discovers and applies that plugin's dated migration partials in chronological order during startup. |
...
| **`ErpPrincipal` / `ErpIdentity`** | The platform's authenticated security context types (`WebVella.Erp.Web/Security/`), used together with `AuthToken`, the custom `AuthorizeAttribute`, and `AuthCache` to enforce authorization. |
